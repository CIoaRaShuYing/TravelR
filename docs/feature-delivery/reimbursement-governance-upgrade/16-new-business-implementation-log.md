...
- 已确认：管理员可以新建归属于本人的周报；普通用户导出本人，管理员导出当前筛选范围内全部周报；Excel 按周从旧到新排列。
- 已确认：周报新建/编辑日期及周区间筛选的日历只能选择周一，其他日期禁用，服务端继续校验周一规则。
- 技术方案：复用现有 `XlsxWorkbookWriter` 和前端下载链路，新增 `WeeklyReportExportService` 及本人/管理员两个导出端点，不修改数据库、不增加依赖。
- 后端：新增本人和管理员导出端点，导出完整筛选结果并写入不含正文的审计；XLSX 按周、项目编码、用户显示名称稳定升序生成。
- 前端：管理员可新建本人周报；新建只显示启用项目，编辑历史记录时保留当前停用项目；导出沿用当前筛选；新建/编辑和周区间筛选日历均禁用非周一日期。
- 自动化验证：`dotnet test backend/TravelReimbursement.Api.Tests/TravelReimbursement.Api.Tests.csproj -c Release --no-restore` 通过 30/30；新增测试覆盖时间顺序、同周稳定顺序、完整正文、中国时区时间和反向区间拒绝。
- 前端验证：`vue-tsc --noEmit --incremental false` 通过；Vite 生产构建通过，仅有既有大分块提示；`git diff --check` 通过。
- 当前状态：NB-08 本地实现和自动化验证完成；未连接真实业务数据库运行 HTTP 导出，也未执行登录态桌面/移动端浏览器验收。
