...
- `GET /api/weekly-reports/export.xlsx`：普通用户按项目和周区间导出本人全部匹配周报。
- `GET /api/admin/weekly-reports/export.xlsx`：管理员按项目、用户和周区间导出全部匹配周报。
...
- 周报页向所有角色显示“新建周报”，管理员新建仍调用本人创建接口；导出按钮按角色调用对应导出端点并沿用当前筛选条件。
- 周报新建/编辑日期及周区间筛选均通过 `disabled-date` 禁用非周一日期；编辑器仅为新建展示启用项目，编辑历史周报时保留当前停用项目。
...
- 新增 `WeeklyReportExportService` 查询完整筛选结果，按 `WeekStart`、项目编码、用户显示名称升序生成单工作表 XLSX；导出端点分别执行本人和管理员权限收敛，并写入不含正文的审计记录。
- 周报导出没有附件，直接返回内存中的 XLSX；不改数据库模型、不增加依赖、不改变现有列表 API。
