...
function canEdit(row: ClaimListRow) { return !row.archiveBatchId && ['Draft', 'Submitted', 'Rejected'].includes(row.status) }
  ...
  <section>
    ...
    <div class="table-shell desktop-table" v-loading="loading">
      <el-table :data="rows" empty-text="当前筛选条件下没有报销。">
        ...
        <el-table-column label="归档" min-width="145"><template #default="scope"><el-tag v-if="scope.row.archiveBatchId" type="info" effect="plain">{{ scope.row.archiveBatchName }}</el-tag><span v-else>-</span></template></el-table-column>
