...
const total = ref(0)
const pageSize = 20
const filters = reactive({ projectId: '', authorId: '', weeks: [] as string[], page: 1 })
...
<template>
  <section>
    <header class="page-header"><div><p class="eyebrow">WEEKLY PROJECT LOG</p><h1>项目周报</h1><p>按项目记录每周完成情况、下周计划和需要协助的问题。</p></div><div class="page-actions"><el-tooltip content="刷新周报"><el-button circle :icon="Refresh" @click="load" /></el-tooltip><el-button v-if="!isAdministrator" type="primary" :icon="Plus" @click="openCreate">新建周报</el-button></div></header>
    <div class="filter-bar">
      ...
      <el-date-picker v-model="filters.weeks" type="daterange" value-format="YYYY-MM-DD" range-separator="至" start-placeholder="周开始" end-placeholder="周结束" @change="applyFilters" />
    </div>
    ...
    <el-dialog v-model="editor.visible" :title="editorTitle" width="min(640px, calc(100vw - 32px))" :close-on-click-modal="false"><el-form label-position="top">
      <el-form-item label="项目" required><el-select v-model="editor.projectId" filterable><el-option v-for="project in projects" :key="project.id" :label="`${project.code} · ${project.name}`" :value="project.id" /></el-select></el-form-item>
      <el-form-item label="周一日期" required><el-date-picker v-model="editor.weekStart" type="date" value-format="YYYY-MM-DD" placeholder="请选择周一" /></el-form-item>
