...
<template>
  <section>
    <header class="page-header"><div><p class="eyebrow">PROJECT MEETING LOG</p><h1>会议记录</h1><p>按项目沉淀会议参与人、需求内容和后续工作重点。</p></div><div class="page-actions"><el-button :icon="Download" @click="openExport">按项目导出</el-button><el-tooltip content="刷新会议记录"><el-button circle :icon="Refresh" @click="load" /></el-tooltip><el-button type="primary" :icon="Plus" @click="openCreate">新建会议记录</el-button></div></header>
    ...
    <div class="table-shell desktop-table" v-loading="loading"><el-table :data="rows" empty-text="当前没有会议记录。">
      ...
      <el-table-column label="内容概览" min-width="260"><template #default="scope"><div class="meeting-summary"><span>{{ scope.row.firstItemContent || '暂无摘要' }}</span><small>{{ scope.row.participantCount }} 人 · {{ scope.row.requirementCount }} 条需求 · {{ scope.row.workFocusCount }} 条重点</small></div></template></el-table-column>
    ...
    <div class="mobile-list" v-loading="loading"><article v-for="item in rows" :key="item.id" class="mobile-record"><div class="mobile-record__head"><div><strong>{{ item.projectName }}</strong><span>{{ item.meetingDate }} · {{ item.location }}</span></div><div><el-button text circle :icon="Edit" @click="openEdit(item)" /><el-button v-if="isAdministrator" text circle type="danger" :icon="Delete" :loading="deletingId === item.id" @click="remove(item)" /></div></div><p>{{ item.firstItemContent || '暂无内容摘要' }}</p><p class="meeting-counts">{{ item.participantCount }} 人 · {{ item.requirementCount }} 条需求 · {{ item.workFocusCount }} 条重点</p></article><el-empty v-if="!loading && rows.length === 0" description="当前没有会议记录" /></div>
